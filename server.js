...
//trying to make a second post type
app.get("/art", function (request, response) {
  try {
    initializeDatastoreOnProjectCreation();
    var posts = datastore.get("articles");
    response.render('index.html', {
      title: "Welcome1a!",
      posts: posts.reverse()
    });
...
app.post("/articles", function (request, response) {
  try {
    // Get the existing posts from the MongoDB and put it into an array called posts
    var articles = datastore.get("articles");
    // We get the contents of the submitted form and append it to the posts array
    articles.push(request.body); // the form data is in request.body because we're using the body-parser library to help make dealing with requests easier
    // We store the updated posts array back in our database posts entry
    datastore.set("articles", articles);
    // And then we redirect the view back to the homepage
    response.redirect("/");
  } catch (err) {
    handleError(err, response);
...
function initializeDatastoreOnProjectCreation() {
  ...
  if (!datastore.get("initialized")) {
    ...
    datastore.set("articles", initialArticles);
...
var initialArticles = [
  {
    title: "Hello friend!",
    body: "This is an article, which is suspiciously similar to a blog."
  },
  {
    title: "Another Post in Articles",
    body: "Today I saw a double rainbow. It was pretty neat."
  }
